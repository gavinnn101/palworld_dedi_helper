...
class RconPacket:
    ...
    def pack(self):
            body_encoded = self.body.encode("ascii") + self.terminator  # The packet body field is a null-terminated string encoded in ASCII
            self.size = len(body_encoded) + 10                          # Only value that can change is the length of the body, so do len(body) + 10.
            return struct.pack("<iii", self.size, self.id, self.type) + body_encoded + self.terminator
...
class SourceRcon:
    def __init__(self, server_ip, rcon_port, rcon_password) -> None:
        self.SERVER_IP = server_ip
        self.RCON_PORT = rcon_port
        self.RCON_PASSWORD = rcon_password


    def create_packet(self, command, request_id=1, type=RCONPacketType.SERVERDATA_EXECCOMMAND):
    ...
    def send_command(self, command):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            auth_packet = self.create_packet(self.RCON_PASSWORD, type=RCONPacketType.SERVERDATA_AUTH)
